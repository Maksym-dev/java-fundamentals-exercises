...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream().filter(account -> account.getBirthday().getMonth().equals(birthdayMonth))
                .toList();
    }
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.partitioningBy(account -> account.getSex() == Sex.MALE));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().collect(Collectors.groupingBy(account -> {
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream().anyMatch(account -> account.getEmail().split("@")[1].equals(emailDomain));
    }
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream().collect(Collectors.toMap(Account::getLastName, account -> {
            Set<String> objects = new HashSet<>();
            objects.add(account.getFirstName());
            return objects;
        }, (strings, strings2) -> {
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        return accounts.stream().collect(Collectors.toMap(account -> account.getBirthday().getMonth(),
                Account::getFirstName, (s, s2) -> s + ", " + s2));
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream().collect(Collectors.toMap(account -> account.getCreationDate().getMonth(),
                Account::getBalance, BigDecimal::add));
    }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream().map(Account::getFirstName)
